...
export default function MobileLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        setMessage({
          type: "error",
          text: "Please use the signup page for creating accounts.",
        });
      }
    } catch (error) {
      setMessage({
  ...
  const getMessageStyles = (type: string) => {
    switch (type) {
      ...
      default:
        return "border-gray-400/50 bg-gray-500/20 text-gray-100"
    }
  }
  ...
  const MotivationalSection = () => (
    <div className="w-full flex flex-col items-center justify-center text-center mb-8 mt-8">
      ...
      <div className="flex flex-wrap justify-center gap-3 pt-4">
        <div className="bg-white/10 backdrop-blur-md rounded-full px-3 py-2 text-xs sm:text-sm font-medium">
          🚀 Advanced Learning
        </div>
        <div className="bg-white/10 backdrop-blur-md rounded-full px-3 py-2 text-xs sm:text-sm font-medium">
          🎓 Expert Instructors
        ...
        <div className="w-full max-w-md mx-auto">
          ...
          <div className="flex flex-col items-center justify-center text-center mb-8">
            <div className="mb-4 transform hover:scale-105 transition-transform duration-500">
              <Link href="/">
                <Logo variant="large" className="w-32 h-auto drop-shadow-2xl animate-pulse cursor-pointer" />
              </Link>
            </div>
          ...
          <Card className="backdrop-blur-md bg-white/10 border-white/20 shadow-2xl rounded-2xl overflow-hidden">
            ...
            <CardContent className="space-y-6">
              {message && (
                <div className={`p-4 rounded-lg border ${getMessageStyles(message.type!)}`}>
