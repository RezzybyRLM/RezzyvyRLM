...
async function testApplyFunctionality() {
  ...
  try {
    ...
    if (!internships || internships.length === 0) {
      ...
      const { data: newInternship, error: internshipError } = await supabase
        ...
        .insert({
          ...
          description: "This is a test internship for testing the apply button functionality.",
          location: "Remote",
    ...
    // Try both tables to see which one works
    const applicationData = {
      student_id: userId,
      internship_id: internshipId,
      application_text: "This is a test application submitted via the test script.",
      status: "pending",
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    }
    ...
    if (applicationError) {
      console.log(`⚠️ Could not insert into internship_applications: ${applicationError.message}`)
      console.log("Trying applications table instead...")

      const { data: altApplication, error: altError } = await supabase
        .from("applications")
        .insert({
          student_id: userId,
          internship_id: internshipId,
          student_statement: applicationData.application_text,
          status: "pending",
      ...
      if (altError) {
        console.error(`❌ Failed to submit application: ${altError.message}`)
        return
      }

      console.log(`✅ Application submitted to applications table: ${altApplication[0].id}`)
    } else {
      console.log(`✅ Application submitted to internship_applications table: ${application[0].id}`)
    }
    ...
    console.log("\n🎉 Apply button functionality test completed successfully!")
    console.log("\n📝 Test credentials:")
